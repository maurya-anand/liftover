...
$(dir)/genomes/hg19.fa:
	...
	@mkdir -p $(dir)/genomes
	@wget --no-check-certificate --no-verbose https://hgdownload.soe.ucsc.edu/goldenPath/hg19/bigZips/hg19.fa.gz -O $(dir)/genomes/hg19.fa.gz
	@gunzip $(dir)/genomes/hg19.fa.gz

$(dir)/genomes/hg38.fa:
	@printf "Downloading hg38.fa\n"
	@mkdir -p $(dir)/genomes
	@wget --no-check-certificate --no-verbose https://hgdownload.soe.ucsc.edu/goldenPath/hg38/bigZips/hg38.fa.gz -O $(dir)/genomes/hg38.fa.gz
	@gunzip $(dir)/genomes/hg38.fa.gz

$(dir)/genomes/hg19ToHg38.over.chain.gz:
	@printf "Downloading hg19ToHg38.over.chain\n"
	@mkdir -p $(dir)/genomes
	@wget --no-check-certificate --no-verbose https://hgdownload.soe.ucsc.edu/goldenPath/hg19/liftOver/hg19ToHg38.over.chain.gz -O $(dir)/genomes/hg19ToHg38.over.chain.gz

$(dir)/tools/libexec/bcftools/liftover.so:
	@printf "Installing liftover\n"
	@mkdir -p $(dir)/tools
	@wget --no-check-certificate --no-verbose https://github.com/samtools/bcftools/releases/download/1.19/bcftools-1.19.tar.bz2 -O $(dir)/tools/bcftools-1.19.tar.bz2
	...
	@wget --no-check-certificate --no-verbose https://software.broadinstitute.org/software/score/score_1.19-dev.zip -O $(dir)/tools/score_1.19-dev.zip
	@unzip $(dir)/tools/score_1.19-dev.zip -d $(dir)/tools/libexec/bcftools
	
	@wget --no-check-certificate --no-verbose https://github.com/samtools/htslib/releases/download/1.19.1/htslib-1.19.1.tar.bz2 -O $(dir)/tools/htslib-1.19.1.tar.bz2
